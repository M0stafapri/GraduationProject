...
class SignCubit extends Cubit<SignStates> {
  ...
  void createUser(
    ...
    await FirebaseAuth.instance
      ...
      await CacheHelper.saveCacheData(
          key: 'uid',
          val: val.user!
              .uid); // to save User ID on Cache to go to home directly second time
      userID = await CacheHelper.getCacheData(key: 'uid');
      saveUserData(
          userName: userName,
          email: email,
          uid: val.user!.uid,
          phoneNumber: phoneNumber); // save UserData on Cloud FireStore
    }).catchError((e) {
  ...
  Future<void> userLogin(
    ...
    try {
      ...
      debugPrint("Error during login to MyFitnessPal, reason is : $error");
